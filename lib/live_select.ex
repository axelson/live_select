defmodule LiveSelect do
  ...
  to send the list of options to LiveSelect. See the ["Examples"](#module-examples) section below for details, and check out the
  ...
  Whenever an option is selected, `LiveSelect` will trigger a standard `phx-change` event in the form. See the ["Examples"](#module-examples) section
  ...
  * See [the styling section](styling.md) for details
  ...
  attr :options, :list,
    doc:
      ~s(initial available options to select from. Note that, after the initial rendering of the component, options can only be updated using `Phoenix.LiveView.send_update/3` - See the ["Options"]\(#module-options\) section for details)
